 ...
 * @param request - the HTTP method and optional body to send; defaults to an
 *   anonymous GET when omitted.
...
export async function requestPinned(
  ...
  try {
    const response = await fetch(url, {
      ...
      signal,
      // proxy-exempt: the pinned lookup agent above, whose lifetime is this one request.
      dispatcher,
    })
