---
description: "The anonymous-by-default public HTTP(S) fetch backend for ctx.web: how deployments mount bounded, safe URL retrieval with same-origin redirects and text-only decoding."
...
Choose this backend when a deployment must fetch public pages with bounded output and safe transport: no credentials are sent unless a caller supplies its own `headers`, every resolved address must be public, each connection is pinned to the validated answer set, redirects cannot escape the origin, and every response is capped.
...
The provider keeps requests anonymous and bounded by default: it accepts only `http:` and `https:` URLs without embedded credentials and rejects URLs over 2,048 characters, and it sends no ambient credentials — a caller may still pass explicit `headers`, including an `Authorization` (see the transport paragraph below and the limitations). It resolves each hostname once, rejects the complete result if any IPv4 or IPv6 address is not public unicast, and pins the connection to that validated set. IPv6 checks discover the active DNS64 prefix and reject translations to non-public IPv4. Each same-origin redirect repeats resolution and pinning; cross-origin redirects fail and require a fresh call. The provider also enforces byte, character, hop, and time caps, rejects unsupported content types, and sends an explicit product `User-Agent`.

A caller may pass `method`, `headers`, and `body` on the request. `method` defaults to `GET`; a `body` supplied with `GET` or `HEAD` is dropped before the transport, which rejects one outright. Caller headers apply over the provider's defaults through `Headers` case-insensitive normalization, so a caller `User-Agent` replaces the default rather than joining it. Redirect method rewriting follows WHATWG `HTTP-redirect fetch`: 301 and 302 rewrite only `POST`, 303 rewrites every method except `GET`/`HEAD`, and 307/308 never rewrite; a rewrite drops the body-describing headers (`Content-Type`, `Content-Encoding`, `Content-Language`, `Content-Location`) with the body. A request carrying `Authorization`, `Cookie`, or `Proxy-Authorization` is refused any redirect before the `Location` is resolved, so the redirect target is never contacted (packages/web/AGENTS.md). A `HEAD` response resolves to an empty text body without consulting `Content-Length` or `Content-Type`, whose values describe a body that never arrives (RFC 9110 §9.3.2).
...
- **Only textual content decodes** — html/xhtml and `text/*` plus JSON/XML families; a body with a missing `Content-Type` or any binary type throws `WEB_UNSUPPORTED_CONTENT_TYPE` (an empty response to a write-origin request — including one rewritten to GET by a redirect — is an empty success, not a type failure), and text-extractable PDF decoding is named deferred work.
...
- **A credential-bearing request cannot be redirected at all** — rather than resending or stripping the header, the provider fails the redirect outright with `WEB_REDIRECT_BLOCKED`. A caller whose endpoint legitimately redirects must reissue the request against the target URL itself. Non-credential headers are resent on each same-origin hop.
- **Credential recognition covers the standard headers only** — `Authorization`, `Cookie`, and `Proxy-Authorization` are recognized and block redirects; a caller credential carried in a custom header (for example `X-API-Key`) is treated as an ordinary header and resent on same-origin hops. Cross-origin redirects are never followed, so a custom credential cannot reach another origin; a caller that wants the redirect block for a custom credential should carry it in `Authorization` instead.
