...
Both calls accept an optional `AbortSignal` that is forwarded to the provider for cancellation. The normalized request and result shapes are the contract callers build on; the vocabulary section of the [web subsystem](../../../docs/subsystems/web.md) reference describes them exhaustively. `WebFetchRequest` carries `url`, `method?`, `headers?`, and `body?`; omitting the three optional fields requests the anonymous GET the seam retrieved before they existed, and `WebFetchMethod` is a closed union so a provider can apply its redirect and body rules to every member. `WebFetchBody` is a closed discriminated union (`html` | `text`) owned here — consumers `switch` to exhaustiveness so a new kind breaks their compilation until handled. See `src/types.ts` for the full contracts and the `WebError` code taxonomy.
