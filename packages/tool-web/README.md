...
`method` defaults to `GET` and is one of `GET`/`POST`/`PUT`/`PATCH`/`DELETE`/`HEAD`; `headers` are name/value string pairs applied over the provider defaults, so a caller `User-Agent` or `Accept` replaces rather than joins the default; `body` is a caller-serialized string and is rejected with `GET` and `HEAD`. A request carrying `Authorization` or `Cookie` is refused any redirect before the target is resolved, so a credential never reaches a second endpoint; `Proxy-Authorization` is refused outright as a transport-managed header, so a proxy credential is never forwarded to the origin. Redirects rewrite the method to a bodyless `GET` exactly where browsers and curl do (301/302 on `POST`, 303 on other body-carrying methods), dropping the body-describing headers with the body.

```text
web_fetch({ url: 'https://example.com' })
web_fetch({ url: 'https://api.example.com/v1/items', method: 'POST', headers: { Authorization: 'Bearer …', 'Content-Type': 'application/json' }, body: '{"name":"x"}' })
```
...
Schema validation rejects an absent or non-array `queries` field, non-string array elements, an oversized array, or a blank URL before execution, with exact messages such as `Error: queries must contain at least one query` and `Error: url must be a non-empty string`. An out-of-enum `method`, a non-string `headers` value, a header name that is not an HTTP token, a transport-managed header name such as `Host` or `Expect` (the transport generates, rewrites, or rejects these itself), a header value containing CR/LF/NUL, or a `body` paired with `GET`/`HEAD` fails the same way, naming the rejected field. Provider-side failures surface as structured error tool results; the model can read them and decide the next step, for example fetching a cited URL or refining a query.
